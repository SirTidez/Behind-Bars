...
namespace Behind_Bars.Systems.Jail
{
    ...
    /// </summary>
    public class ScannerStation : InteractableObject
    {
        [Header("Scanner Mode")]
        ...
        [Header("Palm Scanner Components")]
        ...
        void Start()
        {
            ...
            // ALWAYS set up interaction like MugshotStation
            SetMessage(useNewPalmScanner ? "Press E to scan fingerprints" : "Press E to scan fingerprints");
        ...
        {
            isScanning = true;
            SetMessage("Drag your hand to the scanner...");
            ...
            {
                SetMessage("Fingerprint scan complete");
                SetInteractableState(EInteractableState.Label);
            }
            else
            {
                SetMessage("Press E to scan fingerprints");
        ...
        void Update()
        {
            ...
            if (!isScanning && !isPalmScanning && !inScannerView)
            {
                if (IsComplete())
                {
                    SetMessage(useNewPalmScanner ? "Palm scan complete" : "Fingerprint scan complete");
                    SetInteractableState(EInteractableState.Label);
                }
                else
                {
                    SetMessage(useNewPalmScanner ? "Press E for palm scan" : "Press E to scan fingerprints");
                    SetInteractableState(EInteractableState.Default);
                }
            }
